...
const CommandGroupTypography = styled(Typography)<TypographyProps>(
  ({ theme }) => ({
...
const SelectionTypography = styled(Typography)<TypographyProps>(
  ({ theme }) => ({
      ...
      <TreeItem
        ...
        label={
          ...
          >
            ...
            {command.versions !== undefined &&
              command.selectedVersion !== undefined && (
                <Box
                  sx={{
                    marginLeft: 1,
                    display: "flex",
                ...
                >
                  ...
                  >
                    ...
                    >
                      {command.versions!.map((version) => (
                        <MenuItem
                          value={version.name}
                          key={`${command.id}-version-select-${version.name}`}
                        >
                          <SelectionTypography>
                            {version.name}
                          </SelectionTypography>
                  ...
                  >
                    ...
                    >
                      <MenuItem
                        value={1}
                        key={`${command.id}-register-select-registered`}
                      >
                        <RegisteredTypography>Registered</RegisteredTypography>
                      </MenuItem>
                      <MenuItem
                        value={0}
                        key={`${command.id}-register-select-unregistered`}
                      >
  ...
  ({ commandGroup, onUpdateCommandGroup, onLoadCommands }) => {
    ...
    const onUpdateSubCommandGroup = React.useCallback(
      (
        name: string,
        updater: (
          oldCommandGroup: ProfileCTCommandGroup
    ...
    const updateGroupSelected = (
      ...
      const commands = group.commands
        ? Object.fromEntries(
            Object.entries(group.commands).map(([key, value]) => [
              key,
              updateCommandSelected(value, selected),
            ])
          )
        : undefined;
      const commandGroups = group.commandGroups
        ? Object.fromEntries(
            Object.entries(group.commandGroups).map(([key, value]) => [
              key,
              updateGroupSelected(value, selected),
            ])
          )
        : undefined;
      return {
    ...
    const selectCommandGroup = React.useCallback(
      (selected: boolean) => {
        onUpdateCommandGroup(nodeName, (oldCommandGroup) => {
          ...
          const [loadingNamesList, newGroup] =
            prepareLoadCommandsOfCommandGroup(selectedGroup);
      ...
      <TreeItem
        ...
        label={
          ...
          >
            ...
            />
            <FolderIcon />
            <CommandGroupTypography sx={{ marginLeft: 1 }}>
              {nodeName}
            </CommandGroupTypography>
          </Box>
        }
      >
...
interface CLIModGeneratorProfileCommandTreeProps {
  ...
  onChange: (
    updater:
      | ((oldProfileCommandTree: ProfileCommandTree) => ProfileCommandTree)
      | ProfileCommandTree
  ) => void;
...
const CLIModGeneratorProfileCommandTree: React.FC<
  CLIModGeneratorProfileCommandTreeProps
> = ({ profileCommandTree, onChange, onLoadCommands }) => {
  const [defaultExpanded, _] = React.useState(
    GetDefaultExpanded(profileCommandTree)
  );
...
function decodeProfileCTCommand(
  ...
  const versions = response.versions?.map((value: any) =>
    decodeProfileCTCommandVersion(value)
  );
  const command = {
...
function decodeProfileCTCommandGroup(
  response: CLISpecsCommandGroup,
  selected: boolean = false
): ProfileCTCommandGroup {
  const commands =
...
function BuildProfileCommandTree(
  profileName: string,
  response: CLISpecsCommandGroup
): ProfileCommandTree {
  const commandGroups =
    response.commandGroups !== undefined
      ? Object.fromEntries(
          Object.entries(response.commandGroups).map(([name, group]) => [
            name,
            decodeProfileCTCommandGroup(group),
          ])
        )
      : {};
  return {
...
function prepareLoadCommandsOfCommandGroup(
  commandGroup: ProfileCTCommandGroup
): [string[][], ProfileCTCommandGroup] {
  ...
  const commandGroups = commandGroup.commandGroups
    ? Object.fromEntries(
        Object.entries(commandGroup.commandGroups).map(([key, value]) => {
          const [namesListSub, updatedGroup] =
            prepareLoadCommandsOfCommandGroup(value);
...
function PrepareLoadCommands(
  tree: ProfileCommandTree
): [string[][], ProfileCommandTree] {
  const namesList: string[][] = [];
...
function genericUpdateCommand(
  ...
  const updatedCommand = updater(
    currentCommandGroup.commands![names[names.length - 1]]
  );
  ...
  const groupSelected = calculateSelected(
    commands,
    currentCommandGroup.commandGroups!
  );
  currentCommandGroup = {
    ...currentCommandGroup,
    commands: commands,
  ...
  for (const node of nodes.reverse().slice(1)) {
    const commandGroups = {
      ...node.commandGroups,
      [currentCommandGroup.names[currentCommandGroup.names.length - 1]]:
        currentCommandGroup,
    };
...
function calculateSelected(
  ...
  const commandGroupsAllSelected = Object.values(commandGroups).reduce(
    (pre, value) => {
      return pre && value.selected === true;
    },
    true
  );
  const commandGroupsAllUnselected = Object.values(commandGroups).reduce(
    (pre, value) => {
...
function initializeCommandGroupByModView(
  ...
  const commandGroups =
    simpleCommandGroup.commandGroups !== undefined
      ? Object.fromEntries(
          Object.entries(simpleCommandGroup.commandGroups).map(
            ([key, value]) => [
              key,
              initializeCommandGroupByModView(
                view?.commandGroups?.[key],
                value
              ),
            ]
          )
...
function ExportModViewCommand(
  command: ProfileCTCommand
): CLIModViewCommand | undefined {
...
function ExportModViewCommandGroup(
  commandGroup: ProfileCTCommandGroup
): CLIModViewCommandGroup | undefined {
